...
SCALER_PATH = Path('scaler.pkl') # added scaler path
TABLE = 'stats'
...
@st.cache_resource
def load_model_and_scaler(): # load scaler alongside model
...
# Main app
try:
    model, scaler = load_model_and_scaler() # unpacked both objects
except Exception as e:
    st.error(f"Could not load model or scaler. Run the notebook first. Error: {e}")
    st.stop()
...
with get_conn() as conn:
    ...
    if submitted:
        try:
            ...
            # Transform raw inputs with scaler before prediction
